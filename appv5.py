...
if data:
    ...
    if st.button("AI精密査定を実行"):
        ...
        📍 {selected_loc.replace('東京都','')}</h3><p style="color:#64748b;font-size:13px;">{area}㎡ / 築{2026-year_built}年 / 
        徒歩{walk_dist}分</p><div style="display:flex;flex-wrap:wrap;margin-top:25px;gap:20px;">
        <div style="flex:1;min-width:250px;">
        <div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:14px;">
        <span style="color:#64748b;">地点固有地力 α</span><span style="font-weight:bold;">
        Rank {p['alpha']}</span></div><div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:14px;">
        <span style="color:#64748b;">地点利便性指数 μ</span><span style="font-weight:bold;">Rank {p['mu']}
        </span></div><div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:14px;"><span style="color:#64748b;">面積希少性 λ</span><span style="font-weight:bold;">Rank {p['lambda']}</span>
        </div><div style="display:flex;justify-content:space-between;padding:10px 0;border-bottom:1px solid #f1f5f9;font-size:14px;"><span style="color:#64748b;">時系列動態 γ</span><span style="font-weight:bold;">Rank {p['gamma']}
        ...
        <div style="color:#b45309;font-size:18px;font-weight:bold;letter-spacing:1px;text-align:left;">AI 指値</div>
        ...
        <div style="color:#1e293b;font-size:14px;">Tier 3: {int(std_price * 1.05):,} 円</div></div></div></div><div style="background-color:{status_bg};padding:25px;border-radius:12px;border:3px solid {status_color};margin-top:30px;"><div style="font-family:'Courier New',monospace;font-size:18px;color:{status_color};font-weight:bold;line-height:1.6;">>> ANALYSIS_SEQUENCE_COMPLETE...<br>>> ALPHA_RANK_{p['alpha']}<br>>> MU_RANK_{p['mu']}<br>>> GAMMA_RANK_{p['gamma']}<br>>> LAMBDA_NON_LINEAR_RATIO: {p['lambda']*10}%<br>>> MARKET_INEFFICIENCY_DELTA EVALUATED</div></div></div>"""
